            ...
            <div x-data="imageData()" class="file-input flex items-center">
                ...
                <div class="py-3 rounded-full overflow-hidden bg-gray-100">
                    ...
                    <div x-show="!previewPhoto" class="profile-image mb-1">
                        <img src="{{ isset($latestTech->image) ? setImage($latestTech->image) : asset('backend\assets\images\default\default_image.jpg') }}"
...
<div class="col-lg-12">
    <div class="input-style-1">
        <label for="title">Latest Technews Title <span class="text-sm text-danger">*</span></label>
        <input type="text" name="title" value="{{ $latestTech->title ?? old('title') }}" id="title"
            class="@error('title') is-invalid @enderror" placeholder="LatestTech Title">
        @error('title')
...
<div class="col-lg-12">
    <div class="input-style-3">
        <label for="description">Description <span class="text-sm text-danger">*</span></label>
        <textarea name="description" rows="5" id="description" class="@error('description') is-invalid @enderror">
            {{ $latestTech->description ?? old('description') }}
        </textarea>
        @error('description')
            <div class="alert alert-danger mt-1">{{ $message }}</div>
        @enderror
...
<div class="col-lg-12">
    <div class="form-check form-switch toggle-switch mb-30">
        <input class="form-check-input @error('status') is-invalid @enderror" name="status"
            value="{{ $latestTech->status ?? 1 }}" type="checkbox" id="status"
            @if (isset($latestTech->status) && $latestTech->status == 1) checked @endif>
